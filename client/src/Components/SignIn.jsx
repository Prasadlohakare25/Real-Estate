...
function SignIn() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch('/api/auth/signin', {//we have set the path of the api in vite config file and rest all the routes we calling here (anything that starts with api)
        method: 'POST',
        headers: {
      ...
      if (data.success === false) {
        ...
        // setLoading(false);
        dispatch(signInFailure(data.message))//we have created a function for this so we are not doing the other steps
        return;
      }
      // setLoading(false);
      // setError(null);
    ...
    <div className='bg-gradient-to-b from-slate-100 via-slate-300 to-slate-500 h-[88vh]'>
      ...
      <div className='flex justify-center align-middle p-4'>
        <form onSubmit={handleSubmit} className='border-2 border-slate-800 rounded-md p-5 bg-slate-900 shadow-lg shadow-slate-900 text-slate-50 flex flex-col md:p-10 md:font-medium md:w-1/3'>
          ...
          <button disabled={loading} type='submit' className='mt-6 hover:opacity-90 bg-blue-800 w-full rounded-md py-1'>{loading ? 'Loading...' : 'Sign In'}</button>
          <Oauth />
          <div className='flex'>
